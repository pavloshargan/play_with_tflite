...
static constexpr int32_t kNumMaxDet = 6;
static constexpr int32_t kNumKeyPoint = 17;
...
int32_t PoseEngine::Initialize(const std::string& work_dir, const int32_t num_threads)
{
    ...
    /* 0 - 255 (https://tfhub.dev/google/movenet/multipose/lightning/1) */
...
int32_t PoseEngine::Process(const cv::Mat& original_mat, Result& result)
{
    ...
    for (int32_t i = 0; i < kNumMaxDet; i++) {
        val_float += kNumKeyPoint * 3;
        BoundingBox bbox;
        ...
        bbox.label = "";
        bbox.score = val_float[4];
        bbox.x = static_cast<int32_t>(val_float[1] * crop_w) + crop_x;
        bbox.y = static_cast<int32_t>(val_float[0] * crop_h) + crop_y;
        bbox.w = static_cast<int32_t>((val_float[3] - val_float[1]) * crop_w);
        bbox.h = static_cast<int32_t>((val_float[2] - val_float[0]) * crop_h);
        if (bbox.score >= threshold_confidence_) {
            bbox_list.push_back(bbox);
        }
        val_float += 5; /* [ymin, xmin, ymax, xmax, score] */
    }
    ...
    for (const auto& bbox : bbox_nms_list) {
        ...
        val_float += index * (kNumKeyPoint * 3 + 5);    /* 17 x [y, x, s] + [ymin, xmin, ymax, xmax, score]*/
        KeyPoint keypoint;
